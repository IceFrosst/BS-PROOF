...
| **The aggregation rule itself — VOTE COUNTING** | **MEASURED 2026-08-11, `scripts/vote_counting_investigation.py`. This supersedes the two rows above as the primary defect, and it is a FOUNDER decision.** `assemble` reduces each trial to a direction LABEL, `s_value` maps the label to a number, `score_ecu` averages those numbers — so the effect SIZE the paper reported is extracted, stored, and then discarded before it reaches the score. Counting how many trials individually cleared p<0.05 is vote counting, whose power falls toward **zero** as the constituent trials shrink; it is the specific failure meta-analysis was invented to fix, and supplement literature is exactly the small-trial regime where it fails. Two independent measurements: (1) of the `null_effect` claims on the four "higher is better" showcase outcomes that carry a usable number, **24 of 27 (89%) have a point estimate FAVOURING creatine** — they failed to reach significance individually, they did not find nothing, and each is scored −0.35 as evidence AGAINST; (2) **7 published pooled estimates on muscle strength favour creatine with 95% CIs excluding zero** (SMD 0.28–0.46, or 4.4–11.9 kg as a WMD; verbatim-verified, and an adversarial pass instructed to refute them refuted **0 of 24**). Counted separately and not folded in: one more favours creatine at p = 0.001 but publishes no CI, and one sits at SMD 0.20 [0.00, 0.39], whose lower bound touches zero. An earlier version of this row said 8 with CIs excluding zero, merging the p-value-only estimate into the CI count — the same p-value-is-not-a-size error corrected in the row below, while our pipeline scores that outcome **−15**. So the null share is REAL and our extraction is FAITHFUL — the labels are correct — and what is wrong is reading "this trial alone did not reach significance" as "this trial is evidence the product does not work". Caveats stated rather than buried: the 89% rests on 27 of 139 showcase nulls (the other 112 report no figure we extracted), effect units are heterogeneous so only the SIGN is comparable, and none of this establishes creatine's true effect size. Three non-equivalent candidate fixes are set out in the script's verdict, and **v1.15 + `scripts/null_numbers_experiment.py` decided which is available.** S5 now extracts the numbers on nulls too: point-estimate coverage went **14% -> 62%** on 8 replayed studies, so **(i)** effect-size-weighted `s_value` is unblocked. But CI coverage went only **0% -> 9%**, and that ceiling is the LITERATURE, not us -- **6 of those 8 papers mention a confidence interval zero times in their entire full text**, and the two that do are exactly the two S5 got CIs from (PMC2646129 reports 7, S5 returned 7). So **(ii)**, refusing a measured-underpowered null, is NOT available as stated: it needs the interval and the interval is mostly unpublished. The route to it is to RECONSTRUCT the interval from the point estimate, the p-value and n by standard meta-analytic arithmetic (Altman & Bland) -- arithmetic on reported numbers rather than inference of an unreported one, which the 62% coverage now makes possible, but still a new derivation in the scoring path and therefore a founder call. Watch on the next full run: claim counts churned per study (8->14, 12->8 nulls), so the null share and the population A/B must be re-measured, not assumed stable |
