...
| Item | Status |
|---|---|
| `DOSE_UNASSESSABLE_TERM` = 0.50 — **NEW 2026-08-28, `SCORING_MODEL` v15-dose-unassessable-neutral** | **FOUNDER INSTRUCTION: "rn i think the algo is too strict, make it less strict."** `dose_factor_for` returns `None` for two unrelated reasons and the composite punished both at `effect × DOSE_FACTOR["below_50"]` (0.10). `dose.dose_axis_state` now names three: `assessable` (unchanged), `no_benefit_range` — product dose KNOWN but no benefit trial carried a usable dose → **0.50 neutral**, and `product_dose_unknown` → **still punitive**. The middle case was DOUBLE-COUNTING: when nothing worked anywhere the effect term already prices it, so charging it again in the dose slot punishes the same evidence twice; and where benefit trials exist but none reported a convertible dose it punishes ignorance about our own axis, which is not evidence against a product. The last case is deliberately NOT relaxed because rewarding a withheld dose is **gameable** — two selftests pin that declaring a bad dose beats hiding one, and that an unassessable axis outscores a dose measured far from the evidence. 0.50 is the neutral midpoint of the scale `arcs._unit` already defines (0.5 = "no effect either way"), well below the 1.00 an in-range dose earns, so the 99/100-on-an-untested-product failure behind "silence is not a pass" cannot recur. Measured on omega-3: inflammation_crp 23 → 33; signed scores unchanged (dose is not in `w_study`). `composite` is untouched so the TS ports need no resync, and the rule is applied identically by `assemble` and `product_score`. **This supersedes the 2026-08-12 founder call** ("don't fix that thing we lose") that deliberately conflated the two states. **Still open:** the equal-weight mean over effect/form/dose — reweighting needs a new constant, and the ceiling was 67/100 while a form term could be 0 |
| **The form term discarded near-form evidence — RESOLVED, `SCORING_MODEL` v14-form-transfer-ladder** | **FOUNDER INSTRUCTION 2026-08-28: "make the algorithm less strict but at the same it should be absolutely true."** The composite's form term read `form_match == "exact"` studies ONLY; every other tier contributed **0.0**, so `unspecified` (the paper never printed the form), `different` (a form we know is not yours) and *no evidence at all* were priced identically — collapsing three tiers `FORM_FACTOR` already prices apart (exact 1.00 / salt_family 0.50 / unspecified 0.30 / different 0.15). **NO NEW CONSTANT:** a non-exact study now enters at `FORM_LADDER[design_rank] × FORM_FACTOR[form_match]`, and the term is the best of the exact and transfer ladders — `FORM_FACTOR` exists precisely to price partial form evidence and was simply not being read here. The measurement: omega-3 run `20260828_083440`, `inflammation_crp` — **six `unspecified` RCTs, every one non-negative, four at `s = +1.00`, earned zero form credit, while the single exact-form trial at `s = −0.02` (indistinguishable from zero) set `all_negative_in_form` and zeroed the term.** One marginal trial erased six positive ones; composite 17 → 23. This is the founder's own v5 correction applied one tier out ("a negative pooled verdict is a WARNING, not a reason to discard the positive evidence that exists"). **Held back deliberately:** `different` still earns nothing (it answers a different question — the same reason `product-score` refuses an unscored form); "silence is not a pass" survives because the discount is real (0.24 vs 0.80, a 70% penalty, against the 99/100-on-an-untested-product failure that rule was adopted for); invariant 8 is intact because arc `verdict`/`coverage`/`n_in_form` stay exact-form only; and the **signed score does not move at all** (`APPLY_FORM_IN_WEIGHT = False`). The pinned selftest "an unreported form earns no form credit" was reversed with its reasoning recorded inline, plus six new checks. **Still open and NOT taken:** the dose-silence penalty `MISSING_DOSE_PENALTY = 0.10` treats "nobody looked near your dose" as "your dose is wrong" — an explicit founder decision in CLAUDE.md; relaxing it to neutral reads 27 on the row above, 33 combined with v14. Also open: the composite's **equal-weight mean** over effect/form/dose, which lets an unrecorded form cost a product two thirds of its attainable score (ceiling 67/100 while the form term is 0) — reweighting needs a new constant |
