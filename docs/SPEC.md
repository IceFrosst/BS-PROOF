...
| Anchor bands vs the null penalty | **ANSWERED 2026-08-11, and it closes open item 3.** Anchor #1's +80 floor tolerates 8.6% nulls at −0.7, 11.7% at −0.35, and only **20.0% with every negative penalty removed**. No value of `S_VALUE["null_effect"]` makes those bands reachable on real literature, so the BANDS are what is wrong, not the constant. `docs/anchors.csv` needs re-deriving against what the formula can actually produce |
