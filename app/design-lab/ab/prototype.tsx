...
export default function AbPrototype() {
  ...
  <div className="ab-phone"><div className="ab-status"><b>9:41</b><i /><span>▮▮▮ ▰</span></div><div className={`ab-screen layout-${layout}`}>
    ...
    {layout === "overlap" && <>{photo}<div className="ab-overlap-wrap">{header}{scoreBlock}</div>{tabs}<section className="ab-card">{forWhom}{bars}{gates}</section></>}
    {layout === "split" && <>{header}{tabs}<div className="ab-split">{photo}<div className="ab-split-score">{scoreBlock}</div></div><section className="ab-card">{forWhom}{bars}{gates}</section></>}
  </div></div>
