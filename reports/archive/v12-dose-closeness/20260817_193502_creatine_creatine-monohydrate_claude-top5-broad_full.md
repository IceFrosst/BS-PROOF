...
Generated: **2026-08-17 19:35 UTC**
...
- Model calls: **540** (cache hits 0, failures 171)
- Input tokens: **5,107,380** (fresh 133,315 · cache-write 1,394,940 · cache-read 3,579,125)
- Output tokens: **459,694**
- **Spent on this run: $0.00** — subscription, not metered.
- **API-equivalent cost: $38.965** — what the same work would cost billed per token.
- Per study: **7.6 calls**, **$0.5488** API-equivalent across 71 scored studies.

| Agent | Tier | Model | Calls | Cache hits | Fail | In | Out | API-equiv |
|---|---|---|--:|--:|--:|--:|--:|--:|
| S3 | B | `claude-sonnet-5` | 103 | 0 | 44 | 1,981,248 | 56,741 | $17.850 |
| S4 | B | `claude-sonnet-5` | 99 | 0 | 35 | 424,361 | 37,117 | $1.983 |
| S5 | B | `claude-sonnet-5` | 96 | 0 | 34 | 1,908,328 | 107,732 | $14.658 |
| S6B | C | `claude-sonnet-5` | 83 | 0 | 34 | 223,025 | 61,937 | $1.475 |
| S7 | B | `claude-sonnet-5` | 77 | 0 | 9 | 404,822 | 27,574 | $1.871 |
| S8 | A | `claude-haiku-4-5-20251001` | 82 | 0 | 15 | 165,596 | 168,593 | $1.128 |
...
|---|---:|---:|---:|---:|---|
| S3 | 59 | 12 | 0 | 32 | exit 1: Failed to authenticate: OAuth session expired and could not be refreshed (x10); exit 1: max_turns (x2) |
| S4 | 64 | 7 | 0 | 28 | exit 1: Failed to authenticate: OAuth session expired and could not be refreshed (x4); exit 1: max_turns (x3) |
| S5 | 62 | 9 | 0 | 25 | exit 1: Failed to authenticate: OAuth session expired and could not be refreshed (x7); exit 1: max_turns (x2) |
| S7 | 68 | 3 | 0 | 6 | exit 1: Failed to authenticate: OAuth session expired and could not be refreshed (x3) |
| S8 | 67 | 4 | 0 | 11 | exit 1: Failed to authenticate: OAuth session expired and could not be refreshed (x4) |
...
- S6B: 83 attempts in the cost table but no row in the success table -- its failures are invisible to anyone reading success rates
...
|---|---:|---|---|---|---|---|---:|
| muscle_power | 28 | probably does not work | -0.03 @ 100% | 0.80 (pooled +0.23 @ 55%) | not tested | 63% | 6 |
| muscle_strength | 22 | does not work | -0.10 @ 100% | 0.80 (pooled -0.14 @ 74%) | not tested | 50% | 5 |
| lean_body_mass | 21 | does not work | +0.05 @ 100% | 0.80 (pooled +0.00 @ 83%) | not tested | 47% | 4 |
| energy_levels | 19 | works, but not tested for your product | +0.28 @ 100% | 0.80 (pooled +0.30 @ 62%) | not tested | 37% | 3 |
...
|---|--:|--:|--:|--:|---|
| `doi:103390nu15163567` | +12.04 | 0.3089 | 1.0 | 4 | exact |
| `doi:103390nu16152437` | -5.92 | 0.4341 | -0.35 | 4 | unspecified |
| `doi:103390nu16091324` | -4.74 | 0.3473 | -0.35 | 4 | exact |
| `doi:103390nu16060766` | -2.00 | 0.147 | -0.35 | 4 | different |
| `doi:103390nu18111789` | -1.31 | 0.096 | -0.35 | 4 | unspecified |
...
| Study | points | w (quality) | s (direction) | rank | form |
|---|--:|--:|--:|--:|---|
| `doi:101249mss0000000000003202` | -5.42 | 0.6 | -0.19148936170212766 | 4 | exact |
| `doi:1033549physiolres935323` | +2.27 | 0.1602 | 0.3 | 4 | different |
| `doi:103390nu16162772` | -1.77 | 0.1074 | -0.35 | 4 | unspecified |
| `registry:isrctn83081058` | +1.45 | 0.1025 | 0.3 | 4 | exact |
| `doi:1010801939021120252518408` | -1.31 | 0.0796 | -0.35 | 4 | exact |
| **sum of all 5** | **-4.78** | | | | |

### lean_body_mass — signed +3

| Study | points | w (quality) | s (direction) | rank | form |
|---|--:|--:|--:|--:|---|
| `doi:1033549physiolres935323` | +2.36 | 0.1602 | 0.3 | 4 | different |
| `registry:isrctn83081058` | +1.51 | 0.1025 | 0.3 | 4 | exact |
| `doi:1010801939021120252518408` | -1.37 | 0.0796 | -0.35 | 4 | exact |
| `doi:101249mss0000000000003202` | +0.00 | 0.6 | 0.0 | 4 | exact |
| **sum of all 4** | **+2.50** | | | | |

### energy_levels — signed +10

| Study | points | w (quality) | s (direction) | rank | form |
|---|--:|--:|--:|--:|---|
| `doi:103390nu17111772` | +6.79 | 0.4326 | 0.3 | 4 | exact |
| `doi:103390nu18081192` | +5.02 | 0.1645 | 0.5833333333333334 | 4 | unspecified |
| `doi:103390nu18111789` | -1.76 | 0.096 | -0.35 | 4 | unspecified |
| **sum of all 3** | **+10.05** | | | | |
