...
SCORING_MODEL_HISTORY = {
    "v14-form-transfer-ladder":
        ...
        "A non-exact study enters the ladder at FORM_LADDER[rank] x "
        "FORM_FACTOR[form_match] for `unspecified` (0.30) and `salt_family` "
        "(0.50); `different` (0.15) is still excluded because a form we know is "
        "not yours answers a different question. The term is the best of the "
        "exact and transfer ladders. Founder ask 2026-08-28, 'less strict but "
        "absolutely true'; motivated by omega-3 run 20260828_083440 where six "
        "non-negative unspecified RCTs (four at s=+1.00) scored zero form credit "
        "while one exact-form trial at s=-0.02 set all_negative_in_form and "
        "zeroed the term. NO NEW CONSTANT -- FORM_FACTOR exists to price partial "
        "form evidence and was simply not read here. SIGNED SCORE UNCHANGED "
        "(invariant 8 keeps form out of w_study); only the composite moves. "
        "Arc verdict/coverage/n_in_form stay exact-form only, so invariant 8's "
        "'nobody tested your form' vs 'your form failed' distinction is intact.",
    ...
    "v12-dose-closeness":
        ...
        "close our dose is'). Same 0..1 scale and same role-split as the form "
        "term: the arc's verdict/coverage remain the PICTURE of what trials "
        "near your dose found, closeness is the SCORE input, and direction "
        "lives in the effect term alone. Benefit trials far from your dose "
        "stop voting FOR you and become the yardstick you are measured "
        "against: 20 g loading benefits pushed a 4.4 g product's power "
        "composite to 62 under v11; under v12 they define a ~20 g range the "
        "product is far below (closeness 0.10, composite 44). DELIBERATE LOSS, "
        "founder call: the score no longer distinguishes 'dosed where trials "
        "failed' from 'dosed where nobody looked' -- both are outside the "
        "range that worked; null_range and the arc still show a reader the "
        "difference. KNOWN FRAGILITY, recorded not fixed: the range is a "
        "min-max of benefit doses, so one odd benefit trial at an extreme "
        "dose stretches it, and a range built on 2 trials reads identically "
        "to one built on 15. No benefit range (nothing worked, or no benefit "
        "trial carried a dose) falls back to eff x MISSING_DOSE_PENALTY.",
    "v11-number-coherent":
